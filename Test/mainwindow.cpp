...
void MainWindow::onDisconnectBtn()
{
    ...
    //释放table model
    for (auto & tableModel : openedTableModel)
        delete tableModel;
...
void MainWindow::onFilterBtn()
{
    ...
    QString value = ui->comboBoxVal->currentText();

    this->queryModel = this->sql->selectByValue(this->currentTableModel->tableName(), fieldName, value);

    ui->queryTableView->setModel(this->queryModel);
...
{
    int ok = QMessageBox::warning(this, "FUCK!", "请联系管理员实现该功能（不一定会实现）^_^", QMessageBox::Yes);
}

void MainWindow::THANKS()
{
    QString text = QString("鸣谢：\n \n HMY\n 喝牛奶的波波鱼\n 你妈\n 罗老师\n \n Copyright: MJUAI19");
    int ok = QMessageBox::information(this, "^_^ 鸣谢 ^_^", text, QMessageBox::Yes);
}

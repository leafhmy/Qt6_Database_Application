...
MainWindow::MainWindow(QWidget *parent): QMainWindow(parent)
{
    ...
    connect(ui->queryCardNumberButton, &QPushButton::clicked, this, & MainWindow::onQueryCardNumberButton);

    connect(ui->queryOrderButton, &QPushButton::clicked, this, & MainWindow::onQueryOrderButton);
...
void MainWindow::onLoginSuccess(int userType, QString userName, QString password)
{
    ...
    if (userType == 2)
    {
        int response = sql->sellerLoginIdentification(userName, password);
        if (response == 0)
            QMessageBox::information(this, "-_-||", "账号不存在！", QMessageBox::Yes);

        if (response == -1)
            QMessageBox::information(this, "-_-||", "密码错误！", QMessageBox::Yes);
    ...
    case 1:
        ...
        break;
    case 2:
        this->user = factury->getUser(E_Seller);
        break;
    default:
        break;
    }
...
{
    this->ui->currentSellerLabel->setText("当前售票员：" + this->user->userName);
...
void MainWindow::onQueryAction()
{
    ...
    if (user->userType == 1)
    {
        this->ui->sellQueryOrderView->hide();
    ...
    {
        this->ui->workStackedWidget->setCurrentWidget(ui->guestInfoPage);
        this->ui->sellQueryOrderView->show();
    }

}

void MainWindow::onQueryOrderButton()
{
    QString cardNumber = this->ui->queryOrderCardNumLineEdit->text();
    QSharedPointer<QSqlQueryModel> queryModel = sql->getCustomOrder(cardNumber);
    this->setQueryModel(queryModel);
    this->ui->guestOrderTableView->setModel(queryModel.data());
}
...
void MainWindow::onRefundAction()
{
    ...
    this->ui->cardNumberLineEdit->clear();
}
...
{
    QString sell_operator = this->user->userName;
    QString ticket_type = this->ui->ticketTypeComboBox->currentText();
    QString card_number = this->ui->orderForLineEdit->text();
    bool ok = this->sql->orderTicket(card_number, sell_operator, ticket_type, card_number);
...
void MainWindow::onRefundBtn()
{
    ...
    QString cardNumber = this->ui->cardNumberLineEdit->text();
    bool ok = sql->refund(cardNumber, orderId);
...
void MainWindow::onQueryCardNumberButton()
{
    QString cardNumber = this->ui->cardNumberLineEdit->text();
    // 获取用户所有订单号
    QSharedPointer<QVector<QString> > customOrderId = sql->getCustomOrderId(cardNumber);
